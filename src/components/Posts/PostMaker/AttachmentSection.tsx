...
export default function AttachmentSection({
  ...
  function handleFileInput(event: React.ChangeEvent<HTMLInputElement>) {
    if (
      ...
      attachmentsLocal.length + attachments.length < MAXATTACHMENTS
    ) {
      try {
      ...
      <div className="flex justify-between ">
        ...
        text-primary">
          ...
          <p className="text-secondary text-xs ">
            (optional, add up to {MAXATTACHMENTS} files downloadable by your
            sponsors)
          </p>
        </div>
        <div>
          ...
          <button
            type="button"
            className="px-5 text-action underline 
            font-bold text-lg"
            onClick={() => {
