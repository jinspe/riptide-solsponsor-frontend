...
export default async function mintNftTool(
  metadata: MetadataJson,
  uri: string,
  customerWallet: Wallet
): Promise<void> {
  ...
  const { mint, createMintTx, createAssociatedTokenAccountTx, mintToTx } =
    await actions.prepareTokenAccountAndMintTxs(connection, myPublicKey);
  ...
  const creatorsData = creators.reduce<programs.metadata.Creator[]>(
    (memo, { address, share }) => {
      const verified = address === myPublicKey.toString();
  ...
  const createMetadataTx = new programs.metadata.CreateMetadata(
    ...
    },
    {
      ...
      metadataData,
      updateAuthority: myPublicKey, // destKey,
      mint: mint.publicKey,
